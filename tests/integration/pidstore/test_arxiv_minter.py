...
def test_arxiv_minter_new(app):
    """Test arxiv minter in new record."""
    data = {
    ...
    # get all pids
    pids = PersistentIdentifier.query.filter_by(object_uuid=str(record.id), pid_type='arxiv').all()
    # should be only 2
    assert len(pids) == 2


def test_arxiv_minter_update(app):
    """Test arxiv minter in new record."""
    data = {
        '$schema': 'http://localhost:5000/schemas/records/hep.json',
        'document_type': [
